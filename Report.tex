...
\begin{itemize}
  ...
  \item \textit{The instantaneous charging of the vehicles}. One of the main problem in real fleet management is that current battery has a recharging period of order-of-magnitude of hours. This required time has to be taken into account as a period in which the vehicles are not available.
  \item \textit{The very large vehicle transport capacity}. Vehicles have limited number of seats available to transport passengers. Moreover, current autonomous shuttles on the market have a capacity not better than a few passengers, often less than ten.
    \item \textit{The approximate demand scenarios}. Demand is often simulated either for peak or off-peak hours based on simple models. In our case we want to assess our system for customer bookings as close as possible to realistic scenarios. 
\end{itemize}
