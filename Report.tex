...
With the fast advances in autonomous driving technology, some 
=>envision new cities network
=>talk about citymobile2 and smartshuttle
=>concentrate on fixed line

-Fast advances in autonomous driving technology
-rethink the layout of the cities =>
-Autonomous vehicles (AVs) are expected to revolutionize mobility by turning cars into mobility robots and allowing more dynamic and intelligent forms of public transportation. A multitude of transport services are conceivable with AVs, yet it is largely unclear which ones will prevail.
-that the future of mobility isn’t in autonomous vehicles themselves, but in what they can offer when they’re operated and managed collectively in an integrated ecosystem. Being agnostic to vehicle type or brand empowers BestMile’s platform to fully leverage the flexibility offered by autonomous mobility and further serve its vision. 
-Urban  mobility  is  one  of  the  major  challenges  of  the  21st  century  andmost  cities  have reached their saturation point in terms of road traffic and pollution. Current applied solutions being  neither  sustainable  nor  efficient,  autonomous  vehicles  hold  the  leading  position  to solve these challenges.
-In  the  field  of  passenger  transport,  autonomous  vehicle  applications  are  unlimited.  Low-speed  but  fully  autonomous  shuttles  for  last-mile  or  private  site  solutions  were  the  first  to enter the market. They fulfill a crucial market need for low-cost, efficient and flexible mobility solutions for areas that were inadequately if not un-serviced. The  next  generation  of  autonomous  vehicles  including  autonomous  cars  and  autonomous buses are being piloted all around the world. Applications are ranging from public transport through valet parking to on-demand car and ride-sharing.
...
Current state of the art mathematical models about fleet management optimization receive justified
critics due to their unrealistic assumptions and, thus, their impossible application in real world
problems. An indicative, but not exhaustive, list of theses biased assumption are:
\begin{itemize}
  \item The \textbf{instantaneous state change between \textit{active} and \textit{charging} vehicle mode}. In real
  world indeed, charging stations are sparsely distributed on the geographical area and reaching them
  costs energy to vehicles.
  \item The \textbf{instantaneous increase of fleet vehicle number}. The real latency between the moment an operator
  want to add some extra vehicles to its fleet and the moment these vehicles are actually in operation
  can be extremely large. It depends on the geographic location of the parking space and the velocity of
  the vehicles. Keeping in mind that the client acceptability time \footnote{time a client
  accepts to wait before receiving a service} for a taxi service is in the order of a
  few minutes, it makes in most case irrelevant to try to add a vehicle to a fleet if not done with
  anticipation.
  \item The \textbf{instantaneous charging of the vehicles}. One of the main problem in real fleet management is
  that current battery have time charge of order-of-magnitude of hours. This requiered time have to be taken
  into account as a period in which the vehicles are not available.
  \item The \textbf{unlimited vehicle transport capacity}. Vehicles have limited number of seats available on which they
  can transport passengers. Moreover, current autonomous shuttles on market have a capacity not better
  than a few passengers, often less than ten.
\end{itemize}

All these aspects make current models irrelevant for real fleet management operation. In this thesis
we approach the problem from a more realistic point of view. Specifically, we use a real world simulator
that emulates vehicles moving on a geographical area where are distributed charging spaces, parking spaces and pick-up/drop-off locations.
Most important, this simulator get rid of all the assumption of \textbf{instantaneousness}.

The abandonment of the instantaneousness assumption leads trivially to the introduction
of a new factor, the \textbf{latency}, and its corresponding remedy, the \textbf{anticipation}.

\subsection{Research plan}

This research focus on answering the question of \textit{How much vehicles do I must activate
on my fleet in order to optimize my service, in a real and latency-affected world context?}

The optimization is obviously a matter of point of view, and thus needs to be specified  before
any answer can be drawn. We distinguish two kind of optima:
\begin{itemize}
  \item Client-side optimum, which correspond to the shortest waiting time
  \item Operator-side optimum, which correspond to the lowest energy consumption
\end{itemize}

With these definitions, we study the effect of the anticipation of a variety of algorithms,
from a zero time-horizon \footnote{Time horizon means how far in the future we know the demands}
(no anticipation at all) to a 24h time-horizon.
