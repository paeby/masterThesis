...
\end{enumerate}
\item \textbf{Dynamic fleet size scheduling} For both strategies described above, we define the optimal fleet size $\text{optFleet(dem)} := |V^{o}|$. The dynamic fleet size scheduling strategy fuiior a scheduling horizon $H$ consists then in computing at each hour $t^{*}$ of the scheduling horizon $H$ what is the optimal fleet size $\text{optFleet(dem}_{t^{*}})$ and to create appropriate increase or decrease events.
