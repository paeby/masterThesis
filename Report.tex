...
\begin{enumerate}
\item \textbf{Fixed headway}: the agency establishes a fixed interval ant time at which vehicles come to stations. It is convenient for customers as they have access to an exact schedule, but it is hard to keep the time between vehicles constant as it is vulnerable to external disturbances.
...
\caption{Used simulation framework's parameters }
\label{table:simparameters}
\end{table}
