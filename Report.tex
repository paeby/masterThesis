...
\item The second strategy is to define a level of service to be provided to the passengers and to find the minimum number of vehicles needed to satisfy the service constraint. The idea is to find a function that gives the $avgWaitingTime$ from the demand and the fleet size $avgWait_{dem, |V|}$ and then to find the minimum fleet size $|V{°}|$ required so that the $avgWait_{dem, |V^{°}|}$ is under an acceptable $avgWaitingTime$ threshold denoted $accWaitingTime$. As we have the data for the $avgWaitingTime$ based for each $dem$ and $|V|$ combinations it is possible to run for example linear or non-linear regressions to get the equation for the function $avgWait_{dem, |V|}$. An example of this procedure applied to a real-life scenario is explained in details in section \ref{experiments}.
\end{enumerate}
\item \textbf{Dynamic Fleet Size Scheduling Algorithm} For both strategies described above, we define the optimal fleet size $optFleet(dem) = |V^{°}|$. The dynamic fleet size scheduling strategy for a scheduling horizon $H$ consists then in: 
...
\begin{algorithm}[H]
 ...
 \caption{Dynamic Fleet Size Algorithm}
\end{algorithm}
\end{itemize}
...
\hline	  
  ...
  $V^{-}, V^{+}$ & Simulations between fleet size $V^{-}$ and $V^{+}$ for finding optimal fleet size \\
    ...
    $opt(dem) =| V^{°}|$ & Optimal fleet size $|V^{°}|$ for demand $dem$ \\
...
The purpose of this section is to get insights as close as possible to real-life conditions on a project which is undergoing in Mountain View, California. 
Describe the loop
Navya vehicles properties

different scenarios that we run on our simulation framework described in section \ref{framework}.
