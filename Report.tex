...
\begin{figure}[h] 
  \centering
\includegraphics[scale=0.55]{./images/simulationFramework.png}
\caption{Interconnected modules composing the simulation framework}
\label{fig:simulationFramework}
\end{figure}
Figure~\ref{fig:simulationFramework} illustrates the different web services and how they are linked to each others. In what follows we describe the behaviour of each module and the simulation parameters which can be tuned.
- Deileon simulator for the vehicles => register the vehicles, handle battery, send TCP messages with hermes
- World-simulator starting the vehicles, sending the edge speeds and bookings
- 
\subsubsection*{World Simulator}
The world-simulator is the web service which is used to start the simulation, send the booking requests and send traffic updates information. At the beginning of the simulation, it sets the time control to a given ratio which is broadcasted to the other web services in order to synchronize them. A day can be simulated in $m$ minutes which corresponds to a ratio $r$ of $m / 24h$. The world-simulator is instantiated with a scenario describing the external events which affect the overall system. It mimics the bookings which would have been made by users with a booking application. The scenario is 
